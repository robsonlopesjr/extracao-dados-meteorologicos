...
columns_name_to_drop = ["weather", "weather_icon", "sys.tipe"]
...
def drop_columns(df: pd.DataFrame, columns_name: list[str]) -> pd.DataFrame:
    logging.info(f"\n→ Recomendo colunas: {columns_name} colunas")
...
def normalize_datetime_columns(
    df: pd.DataFrame, columns_names: list[str]
) -> pd.DataFrame:
    logging.info(f"\n→ Convertento colunas para datetime: {columns_names}")
    ...
    logging.info("\n✓ Colunas convetidas para datetime")
